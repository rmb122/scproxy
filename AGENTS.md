...
- Native host networking: Host listeners and numeric TCP destinations whose selected route is direct use native kernel behavior, including half-close. Zero and loopback addresses default to direct, but explicit IP/CIDR rules can select a proxy instead. FakeIP domain connections always use outbound relays, including direct domain routes, and follow the relay EOF/drain policy above. This project has no published-port forwarding path; do not add interception of native half-close behavior to enforce the relay policy on native connections.
